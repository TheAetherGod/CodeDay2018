...
<body>
	<div id = "footerPusher">
		...
		<div id = "frontImg" class = "card" style = "width: 79%;">
			<p style = "font-style: italic; font-size: 16px; text-align: left;">Most Popular Event!<?php
                                ...
                                for($x=0;$x<$eventCount;$x++){
                                    ...
                                    $stmt = $pdo->prepare($sql);
                                    $stmt->execute(["events" => $a[0][$x]]); 
                                    $eventInt = $stmt->rowCount();
                                    if($eventInt>$largest)$eventIDBiggest=$a[0][$x];}

                                $sql = "SELECT * FROM events WHERE eventID=:events";
                                $stmt = $pdo->prepare($sql);
                                $stmt->execute(["events" => $eventIDBiggest]); 
                                $data = array();
                                $data = $stmt->fetchAll();echo '
                        
                                <div id = "Event1" class = "card" style = "width: 80%; height: 300px;">
                                    ...
                                    <div id = "placeHolderDiv" style = "width: 100%; height:60%; display: flex;justify-content: center;">
                                        ...
                                        <br><b style="color:rgb(100, 248, 144);text-shadow: 1px 1px 2px #000;">' , $data[0][1] ,'</b><br><hr>';if($data[0][4]===$data[0][5]):echo 'During ', $data[0][4];
                                                else:echo 'From ',$data[0][4], ' to ', $data[0][5]; endif; echo' at ',$data[0][6],'<br>',$data[0][2],'
                                        </div>	
                                    </div>
                                    ...
                                    <div id = "intrested?Sponsor?Going?" style = "width: 100%; height: 40%; padding:0px;margin:0px;">
                                        <table style = "width: 100%;height:70%;padding:0px;">
